...
test('Test log in functionality with valid credentials', async ({page}) => {
  ...
  await productPage.tshirt.click();
  await productPage.fleeceJacket.click();
  const badgeValue = await cartPage.getBadgeValue();
  expect(badgeValue).toBe('4');
  await productPage.removebackPack.click();
  await productPage.redTshirt.click();
